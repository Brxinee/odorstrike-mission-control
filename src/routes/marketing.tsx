...
import { Metric, PageHead, Pending, Provenance, Td, Th } from "@/components/mc/ui";
...
function MarketingPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHead kicker="Marketing" title="Attribution without fiction" />
      ...
      <div className="overflow-x-auto rounded-md border border-line">
        <table className="w-full min-w-[640px] border-collapse">
          ...
          <tbody>
            {data.rows.map((r) => (
              <tr key={`${r.source}-${r.medium}`} className="border-t border-line">
                <Td>{r.source}</Td>
                <Td>{r.medium}</Td>
